...
contract('BasicVerificationGame query to high step', function(accounts) {
	...
	//Normally the solution hash will be provided by the incentive layer
	let finalStateHash = (await simpleAdderVM.runSteps.call(program, programLength))[1]

	await basicVerificationGame.initGame(gameId, merkleTree.bufToHex(root), finalStateHash, programLength, responseTime, SimpleAdderVM.address, {from: accounts[2]})
	...
	let gameData = await basicVerificationGame.gameData.call(gameId)
	assert(gameData[0].toNumber() + 1 == gameData[2].toNumber())
    })

    it("should perform step verification", async () => {
	let gameData = await basicVerificationGame.gameData.call(gameId)

	let lowStep = gameData[0].toNumber()
	let highStep = gameData[2].toNumber()
	
    	let lowStepState = toResult(await simpleAdderVM.runSteps.call(program, lowStep)).state

    	let highStepIndex = highStep - 1
